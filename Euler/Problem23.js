...
function getFactors(number) {
    ...
    common.removeItemFromArray(number, factors);

    return factors;
}

function isNumberAbundant(number) {
    var factors = getFactors(number);

    var factorSum = common.sumArray(factors);
...
module.exports = {
    solveProblem: async function () {
        ...
        log.info(`${number} isNumberAbundant := ${isNumberAbundant(number)}`);

        var abundantNumbers = [];

        for (var i = 10; i < 28123; i++) {
            if (isNumberAbundant(i)) {
                abundantNumbers.push(i);
            }
        }

        log.warn(`${abundantNumbers}`);
    }
}